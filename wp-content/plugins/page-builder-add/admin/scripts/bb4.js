...
$(document).on("click", ".confirm_template_yes_replace" , function() {
	   ...
	   if (isConfirmTrue_c == true) {
	   	...
	   	try {
			...
			getTemplateRequestURL = popb_admin_vars_data.templateLibURL+'/library/templates/'+popb_selected_template+".json";
			...
			getTemplateLibraryRequestURL = adminAjaxUrl + '?action=popb_get_template&submitNonce='+POPB_data_nonce+'&templateID='+popb_selected_template;
			  ...
			  error: function( xhr, ajaxOptions, thrownError ) {
				...
				$.ajax({
					...
					error: function( xhr, ajaxOptions, thrownError ) {
						...
						jQuery('.popb_install_template_library').css('display','block');
	  
					  	jQuery('.confirm_safemode_no').on('click',function(){
							jQuery('.popb_install_template_library').css('display','none');
							location.reload();
					  	});
...
$(document).ready(function(){
  ...
  $('.rowBlockUpdateBtn').on('click',function(ev){
    ...
    getTemplateRequestURL = adminAjaxUrl + '?action=popb_get_row_blocks&submitNonce='+POPB_data_nonce+'&blockID='+blockName;

	if (popb_admin_vars_data.templateLibActive == 'true') {
		getTemplateRequestURL = popb_admin_vars_data.templateLibURL+'/library/blocks/rowBlocks/'+blockName+''+".json";
	}
