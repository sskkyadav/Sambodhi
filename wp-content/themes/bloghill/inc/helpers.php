...
if( ! function_exists( 'bloghill_check_enable_status' ) ):
	/**
	 * Check status of content.
	 *
	 * @since Bloghill 1.0.0
	 */
  	function bloghill_check_enable_status( $input, $content_enable ){
...
add_filter( 'bloghill_section_status', 'bloghill_check_enable_status', 10, 2 );


if ( ! function_exists( 'bloghill_is_sidebar_enable' ) ) :
	/**
	 * Check if sidebar is enabled in meta box first then in customizer
	 *
	 * @since Bloghill 1.0.0
	 */
	function bloghill_is_sidebar_enable() {
		$options               = bloghill_get_theme_options();
		...
		if ( is_home() ) {
			...
			if ( ! empty( $post_id ) )
				$post_sidebar_position = get_post_meta( $post_id, 'bloghill-sidebar-position', true );
			else
...
function bloghill_section_tooltip( $section_id ){
	$section_name = ''; 
...
?>	
		<span class="tooltiptext"> <?php echo esc_html( $section_name, 'bloghill' ); ?> </span>
	    <style type="text/css">
...
if ( ! function_exists( 'bloghill_is_frontpage_content_enable' ) ) :
	/**
	 ...
	 */
	function bloghill_is_frontpage_content_enable( $status ) {
		if ( is_front_page() ) {
			$options = bloghill_get_theme_options();
...
add_filter( 'bloghill_filter_frontpage_content_enable', 'bloghill_is_frontpage_content_enable' );

add_action( 'bloghill_action_pagination', 'bloghill_pagination', 10 );
if ( ! function_exists( 'bloghill_pagination' ) ) :
	 ...
	 * @since Bloghill 1.0.0
	 */
	function bloghill_pagination() {
		$options = bloghill_get_theme_options();
		if ( true == $options['pagination_enable'] ) {
			$pagination = $options['pagination_type'];
			if ( $pagination == 'default' ) :
				the_posts_navigation( array(
			'prev_text'	=> bloghill_get_svg( array( 'icon' => 'up' ) ) .  '<span>' . esc_html__( 'Older', 'bloghill' ) . '</span>',
            'next_text' => '<span>' . esc_html__( 'Next', 'bloghill' ) . '</span>' . bloghill_get_svg( array( 'icon' => 'up' ) ),
		) );
...
if ( ! function_exists( 'bloghill_post_pagination' ) ) :
	...
	function bloghill_post_pagination() {
		the_post_navigation( array(
			'prev_text'	=> bloghill_get_svg( array( 'icon' => 'up' ) ) .  '<span>%title</span>',
            'next_text' => '<span>%title</span>' . bloghill_get_svg( array( 'icon' => 'up' ) ),
		) );
	}
endif;
...
function bloghill_get_svg( $args = array() ) {
	 ...
	 *
	 * Example 1 with title: <?php echo bloghill_get_svg( array( 'icon' => 'arrow-right', 'title' => __( 'This is the title', 'textdomain' ) ) ); ?>
	 *
	 * Example 2 with title and description: <?php echo bloghill_get_svg( array( 'icon' => 'arrow-right', 'title' => __( 'This is the title', 'textdomain' ), 'desc' => __( 'This is the description', 'textdomain' ) ) ); ?>
	 *
	...
	if ( 'social' === $args->theme_location ) {
		foreach ( $social_icons as $attr => $value ) {
			if ( false !== strpos( $item_output, $attr ) ) {
				$item_output = str_replace( $args->link_after, '</span>' . bloghill_get_svg( array( 'icon' => esc_attr( $value ) ) ), $item_output );
			}
		}
	}
...
function bloghill_menu_fallback_cb( $args ){
    ...
    $link = $args['link_before']
        	. '<a href="' .esc_url( admin_url( 'nav-menus.php' ) ) . '">' . $args['before'] . esc_html__( 'Add a menu','bloghill' ) . $args['after'] . '</a>'
        	. $args['link_after'];
...
function bloghill_custom_header_banner_title() {	
	...
	<?php elseif ( bloghill_is_blog_page() || is_singular() ): ?>
		<h2 class="page-title"><?php single_post_title(); ?></h2>
	<?php elseif ( is_archive() ) : 
		the_archive_title( '<h2 class="page-title">', '</h2>' );
	elseif ( is_search() ) : ?>
		<h2 class="page-title"><?php printf( esc_html__( 'Search Results for: %s', 'bloghill' ), get_search_query() ); ?></h2>
	<?php elseif ( is_404() ) :
		echo '<h2 class="page-title">' . esc_html__( 'Oops! That page can&#39;t be found.', 'bloghill' ) . '</h2>';
	endif;
}
