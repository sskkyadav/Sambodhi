...
class Youzify_Membership_Social {
	...
	function process_authentication( $query ) {
		...
		if ( is_user_logged_in() || ! isset( $query->query_vars['youzify-authentication'] ) ) {
	  	...
	  	if ( $query->query_vars['youzify-authentication'] != 'social-login' ) {
	  		return;
	  	}
	    ...
	    if ( empty( $provider ) || ! in_array( $provider, youzify_get_social_login_providers() ) ) {
	    	// Redirect User.
	...
	public function wpmu_is_user_need_confirmation( $email ) {
		...
		// Get Result
		$result = $wpdb->get_var( $wpdb->prepare( "SELECT id FROM " . $wpdb->prefix . "youzify_social_login_users WHERE email = %s AND user_id = 0", $email ) );
	...
	public function create_new_user( $user, $provider ) {
		...
		if ( ! empty( $required_fields ) ) {
			...
			// Store User Data.
			youzify_membership_user_session_data( 'set', $required_fields );
			// Redirect to complete Registration Page.
			wp_safe_redirect( youzify_membership_page_url( 'complete-registration' ) );
			exit;
		}
	...
	public function get_user_data( $adapter, $provider ) {
		...
		$user_profile_data = youzify_membership_user_profile_data( 'get' );

		if ( empty( $user_profile_data ) && ! empty( $user_profile ) ) {
			// Set Provider.
			// $user_profile->provider = $provider;
			youzify_membership_user_profile_data( 'set', $user_profile );
		}
	...
	public function get_user_by_provider( $provider, $uid, $email = null ) {
		...
		// Get SQL.
		$sql = "SELECT user_id FROM " . $wpdb->prefix . "youzify_social_login_users WHERE provider = %s AND identifier = %s";
	...
	function get_user_verified_email( $email ) {
		...
		global $wpdb;
		// Get SQL.
		$sql = "SELECT user_id FROM " . $wpdb->prefix . "youzify_social_login_users WHERE emailverified = %s";
		// Get Result
	...
	public function bp_register_user( $user, $provider ) {
		...
		if ( is_wp_error( $user_id ) ) {
			...
			// Add ERROR
			$errors[] = youzify_membership_get_message( $user_id->get_error_message() );
		...
		// Get Required Fields.
		$get_session = json_decode( youzify_membership_user_session_data( 'get' ) );
		...
		if ( youzify_social_registration_needs_activation() ) {
			if ( bp_registration_needs_activation() ) {
				$register_page = youzify_membership_page_url( 'register' );
				$this->redirect( 'registration_needs_activation', $register_page, 'success' );
			}
		}

		do_action( 'youzify_after_social_signup_user', $user_id );
	...
	public function complete_registration( $username, $email, $provider ) {
		...
		// Init Vars.
		$errors = new WP_Error();
		$error_msg = $Youzify_Membership->register;
		...
		if ( is_wp_error( $user_id ) ) {
			...
			$this->redirect( $errors, youzify_membership_page_url( 'complete-registration' ) );
		} else {
	...
	public function get_unique_username( $display_name, $user_login = null ) {
		...
		$separator = apply_filters( 'youzify_social_login_registration_username_separator',  '-' );
	...
	function display_inactive_account_msg( $user_login = null ) {
		...
		// Get Message.
		$messages[] = youzify_membership_get_message( $message, 'error' );

		// Get Messages.
		youzify_membership_add_message( $messages, 'info' );
	...
	public function authenticate_user( $user_id, $hybridauth = null ) {
		...
		// Get Redirection Url.
		$redirection_url = $Youzify_Membership->login->redirect_after_login( null, null, $user );
	...
	/**
	 * Clear Membership Session's.
	 */
	public function clear_session() {

		// Clear Sessions.
		if ( isset( $_COOKIE['HYBRIDAUTH::STORAGE'] ) ) {
			unset( $_COOKIE['HYBRIDAUTH::STORAGE'] );
		}

		youzify_membership_user_session_data( 'delete' );
		youzify_membership_user_profile_data( 'delete' );
	...
	public function get_config( $provider ) {
		...
		// Get Config Data.
		$config = array(
	        'callback'   => home_url( 'youzify-auth/social-login/' . $provider ),
	        'debug_mode' => apply_filters( 'youzify_social_login_debug_mode', false ),
		...
		// Get Network status
		$network_status = youzify_option( 'youzify_' . $network . '_app_status' );
		...
		// Get Networks Params.
		$config['keys'] = array(
			'secret' 	=> youzify_option( 'youzify_' . $network . '_app_secret' ),
		);

		if ( $network == 'twitter' ) {
			$config['keys']['key'] = youzify_option( 'youzify_' . $network . '_app_key' );
		} else {
			$config['keys']['id'] = youzify_option( 'youzify_' . $network . '_app_key' );
		}
    ...
    function get_inactive_account_message( $user_login = null ) {
    	...
    	// Get Inactive Account Message.
		$inactive_account_msg = apply_filters( 'youzify_get_inactive_account_message', __( 'Your account has not been activated. Check your email for the activation link.', 'youzify' ) );
	...
	public function store_user_data( $user_id = null, $provider, $profile ) {
		...
		youzify_update_user_profile_meta( $user_id, $profile );

		// Update User Avatar.
		if ( isset( $profile->photoURL ) && ! empty( $profile->photoURL ) ) {

			// Upload Image Localy.
			$profile_photo = youzify_upload_image_by_url( $profile->photoURL );
			...
			if ( is_multisite() ) {
				update_user_option( $user_id, 'youzify_social_avatar', $profile->photoURL );
			} else {
				update_user_meta( $user_id, 'youzify_social_avatar', $profile->photoURL );
			}
	...
	public function complete_social_registration() {
	    ...
	    $user_session_data = json_decode( youzify_membership_user_session_data( 'get' ) );
		...
		$user_profile_data = youzify_membership_user_profile_data( 'get' );
	 	...
	 	if ( empty( $user ) ) {
	        ...
	        // Display Error.
			$errors[] = youzify_membership_get_message( $Youzify_Membership->form->get_error_message( 'cant_connect' ) );
		...
		if ( ! empty( $account_details['errors']->errors['user_name'] ) ) {
			$errors[] = youzify_membership_get_message( $account_details['errors']->errors['user_name'][0] );
		}

		if ( ! empty( $account_details['errors']->errors['user_email'] ) ) {
			$errors[] = youzify_membership_get_message( $account_details['errors']->errors['user_email'][0] );
		}
		...
		if ( ! empty( $errors ) ) {
			...
			$provider = is_object( $user_session_data ) ? $user_session_data->provider :$user_session_data['provider'];
			...
			// Redirect.
			bp_core_redirect( youzify_membership_page_url( 'register' ));
	...
	public function get_user_profile( $user_id, $provider, $uid ) {
		...
		// Get SQL Request.
		$sql = "SELECT * FROM " . $wpdb->prefix ."youzify_social_login_users WHERE user_id = %d AND provider = %s AND identifier = %s";
	...
	public function redirect( $code, $redirect_to = null, $type = null ) {
		...
		// Get Redirect Url.
		$redirect_url = ! empty( $redirect_to ) ? $redirect_to : youzify_membership_page_url( 'login' );
