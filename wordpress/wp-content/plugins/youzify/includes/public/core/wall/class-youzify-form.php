...
class Youzify_Activity_Form {
	...
	function groups_post_update( $args = '' ) {
		...
		$activity_action  = sprintf( __( '%1$s posted an update in the group %2$s', 'youzify' ), bp_core_get_userlink( $user_id ), '<a href="' . bp_get_group_permalink( $bp->groups->current_group ) . '">' . esc_attr( $bp->groups->current_group->name ) . '</a>' );
