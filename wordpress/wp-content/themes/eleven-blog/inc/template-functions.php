 ...
 * @return array
 */
function eleven_blog_body_classes( $classes ) {
...
add_filter( 'body_class', 'eleven_blog_body_classes' );

/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function eleven_blog_pingback_header() {
	if ( is_singular() && pings_open() ) {
		printf( '<link rel="pingback" href="%s">', esc_url( get_bloginfo( 'pingback_url' ) ) );
	}
}
add_action( 'wp_head', 'eleven_blog_pingback_header' );

/**
 * eleven_blog Excerpt Length
 *
 * @since eleven_blog 1.0.0
 *
 * @param null
 * @return void
 */

if ( ! function_exists( 'eleven_blog_excerpt_length' ) ) :
   ...
   */
  function eleven_blog_excerpt_length( $length ) {
...
add_filter( 'excerpt_length', 'eleven_blog_excerpt_length', 999 );